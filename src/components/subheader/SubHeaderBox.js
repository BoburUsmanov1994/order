...
import {get,find,isEqual,sum,round} from "lodash";
    ...
    return (
        <StyledSubHeaderBox {...props}>
            <Row>
                ...
                <Col xs={3}>
                    <SubHeaderItem title={'Янги берилган ордерлар'} percent={round(get(find(get(items,'persrnage'),item => isEqual(get(item,'name'),'янги берилган')),'per','-'),2)} count={get(find(get(items,'orederscount'),item => isEqual(get(item,'statusname'),'янги берилган')),'statuscount','-')} subtitle={'Охирги янгиланиш (4 кун)'} info  hasBorderLeft/>
                </Col>
                <Col xs={3}>
