...
const StatusOrderContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateStatusOrder(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateStatusOrder(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteStatusOrder(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
const mapStateToProps = (state) => {
    return {
        ...
        items: get(state, 'normalizer.data.item-list.result.statusOrder', []),
...
const mapDispatchToProps = (dispatch) => {
    return {
        getItemsList: ({page = 0, size = 20}) => {
            ...
            const scheme = {statusOrder: [RankScheme]};
            dispatch({
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(StatusOrderContainer));
