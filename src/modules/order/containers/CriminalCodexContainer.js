...
const CriminalCodexContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateCriminalCodex(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateCriminalCodex(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteCrimnalCodex(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(CriminalCodexContainer));
