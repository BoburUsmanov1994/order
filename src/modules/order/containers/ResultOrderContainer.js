...
const ResultOrderContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateResultOrder(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateResultOrder(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteResultOrder(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Ҳимоя ордери бериш натижаси</Title>
                </Col>
...
const mapStateToProps = (state) => {
    return {
        ...
        items: get(state, 'normalizer.data.item-list.result.resultorder', []),
...
const mapDispatchToProps = (dispatch) => {
    return {
        getItemsList: ({page = 0, size = 20}) => {
            ...
            const scheme = {resultorder: [RankScheme]};
            dispatch({
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(ResultOrderContainer));
