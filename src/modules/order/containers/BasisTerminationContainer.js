...
const BasisTerminationContainer = ({
                                      history,
                                      getItemsList,
                                      entities,
                                      items,
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateBasisTermination(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateBasisTermination(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteBasisTermination(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Ҳимоя ордерини тугатиш асослари</Title>
                </Col>
...
const mapStateToProps = (state) => {
    return {
        ...
        items: get(state, 'normalizer.data.item-list.result.basistermination', []),
...
const mapDispatchToProps = (dispatch) => {
    return {
        getItemsList: ({page = 0, size = 20}) => {
            ...
            const scheme = {basistermination: [RankScheme]};
            dispatch({
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(BasisTerminationContainer));
