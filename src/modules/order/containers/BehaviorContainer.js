...
const BehaviorContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateBehavior(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateBehavior(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteBehavior(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ёки зўравонлик содир этган, содир этишга мойил бўлган шахсларни ҳулқ-атворини тузатиш дастуридан ўтиш учун юборилган</Title>
                </Col>
...
const mapStateToProps = (state) => {
    return {
        ...
        items: get(state, 'normalizer.data.item-list.result.Agess', []),
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(BehaviorContainer));
