...
const AdministrativeContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateAdministrative(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateAdministrative(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteAdministrative(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(AdministrativeContainer));
