...
const AdministrativeCodexContainer = ({
                                     history,
                                     getItemsList,
                                     entities,
                                     items,
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateAdministrativeCodex(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateAdministrativeCodex(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteAdministrativeCodex(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getOneItem: ({item_id}) => dispatch({
            ...
            payload: {
                url: `/administrativecodex/${item_id}`,
                config: {
                    params: {},
                },
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(AdministrativeCodexContainer));
