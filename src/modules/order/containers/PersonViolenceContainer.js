...
const PersonViolenceContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreatePersonViolence(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdatePersonViolence(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeletePersonViolence(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ёки зўравонлик содир этган шахс</Title>
                </Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(PersonViolenceContainer));
