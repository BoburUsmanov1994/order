...
const OccuredRepetitionContainer = ({
                                              history,
                                              getItemsList,
                                              entities,
                                              items,
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateOccuredRepetition(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateOccuredRepetition(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteOccuredRepetition(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ва зўравонлик ҳолатларини содир этилганлигини такрорийлиги</Title>
                </Col>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getOneItem: ({item_id}) => dispatch({
            ...
            payload: {
                url: `/occurredrepetition/${item_id}`,
                config: {
                    params: {},
                },
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(OccuredRepetitionContainer));
