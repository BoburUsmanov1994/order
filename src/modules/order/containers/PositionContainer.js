...
const PositionContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreatePosition(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdatePosition(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeletePosition(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Шахснинг ҳолати бўйича рўйхати</Title>
                </Col>
...
const mapStateToProps = (state) => {
    return {
        ...
        items: get(state, 'normalizer.data.item-list.result.position', []),
        item: get(state, 'normalizer.data.get-one-item.result.position', {}),
...
const mapDispatchToProps = (dispatch) => {
    return {
        getItemsList: ({page = 0, size = 20}) => {
            ...
            const scheme = {position: [RankScheme]};
            dispatch({
        ...
        getOneItem: ({item_id}) => dispatch({
            ...
            payload: {
                url: `/position/${item_id}`,
                config: {
                    params: {},
                },
                scheme: {position: RankScheme},
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(PositionContainer));
