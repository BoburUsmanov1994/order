...
const ConditionPersonViolenceContainer = ({
                                           history,
                                           getItemsList,
                                           entities,
                                           items,
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateConditionPersonViolence(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateConditionPersonViolence(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteConditionPersonViolence(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ва зўравонлик содир этган шахс ҳолати</Title>
                </Col>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getOneItem: ({item_id}) => dispatch({
            ...
            payload: {
                url: `/conditionpersonviolence/${item_id}`,
                config: {
                    params: {},
                },
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(ConditionPersonViolenceContainer));
