...
const RestrictionOfTypeContainer = ({
                                     history,
                                     getItemsList,
                                     entities,
                                     items,
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateRestrictionOfType(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateRestrictionOfType(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteRestrictionOfType(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ўтказган ёки зўравонлик содир этган ёки содир этишга мойил бўлган шахсга белгиланган чекловлар</Title>
                </Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(RestrictionOfTypeContainer));
