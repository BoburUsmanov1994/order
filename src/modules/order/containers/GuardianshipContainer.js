...
const GuardianshipContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateGuardianship(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateGuardianship(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteGuardianship(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Вояга етмаган ёки суд томондан муомалага лаёқатсиз деб топилган тазйиқ ва зўравонликдан жабрланувчиларга нисбатан ҳимоя ордери кимлар иштирокида расмийлаштириб берилган</Title>
                </Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(GuardianshipContainer));
