...
const FamilyResultContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateResultConflict(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateResultConflict(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteResultConflict(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(FamilyResultContainer));
