...
const ViolenceTypeContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateViolenceType(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateViolenceType(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteViolenceType(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ва зўравонлик турлари</Title>
                </Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(ViolenceTypeContainer));
