...
const CitizenshipContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateCitizenship(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateCitizenship(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteCitizenship(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Фуқаролиги рўйхати</Title>
                </Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(CitizenshipContainer));
