...
const ActionPersonViolenceContainer = ({
                                    history,
                                    getItemsList,
                                    entities,
                                    items,
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateActionPersonViolence(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateActionPersonViolence(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteActionPersonViolence(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
    ...
    return (
        <>
            ...
            <Row className={'mb-24'} align={'center'}>
                <Col xs={6}>
                    <Title>Тазйиқ ва зўравонлик ҳолатлари ким томондан содир этилганлиги</Title>
                </Col>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getOneItem: ({item_id}) => dispatch({
            ...
            payload: {
                url: `/ectionspersonviolence/${item_id}`,
                config: {
                    params: {},
                },
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(ActionPersonViolenceContainer));
