...
const AccountRolesContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateAccountRole(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateAccountRole(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteAccountRole(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(AccountRolesContainer));
