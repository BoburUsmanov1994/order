...
const RanksContainer = ({
    ...
    const create = (params) => {
        setLoading(true);
        ApiService.CreateRank(params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
    ...
    const update = (params) => {
        setLoading(true);
        ApiService.UpdateRank(item_id, params).then((res) => {
            if (res && res.data) {
                setLoading(false);
                setShow(false);
                toast.success('SUCCESS');
            ...
            buttons: [
                {
                    ...
                    onClick: () => {
                        setLoading(true);
                        ApiService.DeleteRank(id).then((res) => {
                            if (res && res.data) {
                                setLoading(false);
                                toast.success('SUCCESS');
                                getItemsList({...filter});
...
const mapStateToProps = (state) => {
    return {
        ...
        items: get(state, 'normalizer.data.item-list.result.zvaniya', []),
        item: get(state, 'normalizer.data.get-one-item.result.zvaniya', {}),
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(RanksContainer));
